...
export class ProductsService {
    ...
    async create(createProductDto: CreateProductDto): Promise<ProductResponseDto> {
        ...
        if (existingProduct) {
            throw new BadRequestException(`Ya existe un producto con el nombre "${createProductDto.name}"`);
        }
        ...
        if (createProductDto.stockGroupId) {
            ...
            if (!stockGroup || !stockGroup.isActive) {
                throw new BadRequestException('El grupo de stock no existe o no está activo');
            }

            // Si se proporciona stockGroupId, no se debe proporcionar individualStock
            if (createProductDto.individualStock !== undefined && createProductDto.individualStock !== null) {
    ...
    async findAll(query: ListProductsQueryDto): Promise<PaginatedProductResponseDto> {
        const { page = 1, limit = 10, search, categoryId, stockGroupId, isAvailable, hasIndividualStock } = query;
    ...
    async findByCategory(categoryId: string, page: number = 1, limit: number = 10): Promise<PaginatedProductResponseDto> {
    ...
    async update(id: string, updateProductDto: UpdateProductDto): Promise<ProductResponseDto> {
        ...
        if (updateProductDto.name) {
            ...
            if (existingProduct) {
                throw new BadRequestException(`Ya existe un producto con el nombre "${updateProductDto.name}"`);
            }
        }
    ...
    async getStats(): Promise<any> {
        ...
        return {
            ...
            categoryCounts,
            availabilityRate: totalProducts > 0 ? Math.round((availableProducts / totalProducts) * 100) : 0,
        };
    }

    async getAvailableProducts(categoryId?: string, search?: string, limit: number = 50): Promise<ProductResponseDto[]> {
        const where: any = {
            isAvailable: true,
            isActive: true,
            deletedAt: null,
