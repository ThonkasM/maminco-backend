...
export class PrintingService {
  ...
  /**
   * Resuelve y valida el nombre de impresora contra la lista blanca del sistema.
   * Evita que un valor controlado por el usuario llegue al comando `lp`.
   */
  private async resolvePrinter(printerName?: string): Promise<string> {
    const available = await this.listPrinters();

    if (printerName) {
      if (!available.includes(printerName)) {
        throw new BadRequestException(`Impresora no válida: ${printerName}`);
      }
      return printerName;
    }

    const defaultPrinter = await this.getDefaultPrinter();
    if (!defaultPrinter || !available.includes(defaultPrinter)) {
      throw new NotFoundException('No se encontró impresora disponible');
    }
    return defaultPrinter;
  }

  /**
   * Envía un buffer a la impresora usando `lp` con argumentos tipo array
   * (sin shell, sin interpolación de comandos).
   */
  private async sendToPrinter(
    ...
    const tmpFile = path.join(this.tmpDir, `${prefix}-${randomUUID()}.bin`);
  ...
  async printTest(
    ...
    try {
      ...
      await this.sendToPrinter(printer, buffer, 'ticket-test');
  ...
  private createOrderTicket(orderData: {
    ...
    encoder.newline().text('-----------------------').align('right');
  ...
  async printOrder(
    ...
    try {
      ...
      await this.sendToPrinter(
        printer,
        buffer,
        `ticket-order-${orderData.orderNumber}`,
      );
  ...
  private createPaymentReceipt(receiptData: {
    ...
    // Subtotal de orden
    encoder
      .text(`Subtotal: Bs ${receiptData.orderSubtotal.toFixed(2)}`)
      .newline();

    // Descuento de orden (si existe)
    if (receiptData.orderDiscount && receiptData.orderDiscount > 0) {
      encoder
        .text(`Descuento: -Bs ${receiptData.orderDiscount.toFixed(2)}`)
        .newline();
    }
    ...
    if (receiptData.totalTip && receiptData.totalTip > 0) {
      encoder
        .text(`Propina Total: Bs ${receiptData.totalTip.toFixed(2)}`)
        .newline();
    }
  ...
  async printPaymentReceipt(
    ...
    try {
      ...
      await this.sendToPrinter(
        printer,
        buffer,
        `receipt-payment-${receiptData.orderNumber}`,
      );
  ...
  async printPaymentReceiptFromDatabase(
    ...
    try {
      ...
      const totalApplied = formattedPayments.reduce(
        (sum, p) => sum + p.applied,
        0,
      );
      const totalChange = formattedPayments.reduce(
        (sum, p) => sum + (p.change || 0),
        0,
      );
      const totalTip = formattedPayments.reduce(
        (sum, p) => sum + (p.tip || 0),
        0,
      );
