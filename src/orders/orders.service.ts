...
export class OrdersService {
     ...
     */
    private calculateTotal(subtotal: number, discount: number, tip: number): number {
    ...
    async create(dto: CreateOrderDto): Promise<OrderResponseDto> {
        ...
        let initialItems: Prisma.OrderItemCreateWithoutOrderInput[] = [];
        let subtotal = 0;
        ...
        const order = await this.prisma.order.create({
            data: {
                ...
                attendedBy: dto.attendedById ? { connect: { id: dto.attendedById } } : undefined,
            ...
            include: {
                ...
                items: {
                    include: { product: true, addedBy: { select: { id: true, name: true } } },
                },
            },
        });
    ...
    async changeStatus(
        ...
        const order = await this.prisma.order.findUnique({
            where: { id },
            include: { items: true, createdBy: true, table: { include: { area: true } } },
        });
        ...
        if (!validTransitions[order.status].includes(dto.status)) {
            const message = order.status === 'CERRADO' && dto.status === 'CERRADO'
        ...
        if (dto.status === 'CERRADO' && order.items.length === 0) {
            throw new BadRequestException(
                'No se puede cerrar una orden sin items',
            );
        }
        ...
        const updated = await this.prisma.order.update({
            ...
            data: {
                status: dto.status,
                closedAt,
                closedBy: dto.status === 'CERRADO' ? { connect: { id: userId } } : undefined,
                histories: {
                    create: {
        ...
        if (dto.status === 'CERRADO') {
            ...
            if (dto.autoPrint) {
                this.logger.log(`Imprimiendo orden #${updated.orderNumber} automáticamente`);
                ...
                try {
                    ...
                    if (printResult.success) {
                        this.logger.log(`Orden #${updated.orderNumber} impresa exitosamente`);
                    } else {
                        this.logger.warn(`Error imprimiendo orden #${updated.orderNumber}: ${printResult.message}`);
                    }
                } catch (error) {
                    this.logger.error(`Error durante impresión automática de orden #${updated.orderNumber}:`, error);
                }
            }
        }
    ...
    async applyDiscount(
        ...
        if (dto.percentageDiscount) {
            ...
            if (dto.fixedDiscount < 0) {
                throw new BadRequestException('El descuento fijo no puede ser negativo');
            }
    ...
    async getRevenueStats(
        ...
        const totalTip = orders.reduce((sum, order) => sum + Number(order.tipAmount), 0);
