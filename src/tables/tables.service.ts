...
export class TablesService {
    ...
    async update(
        ...
        if (
            ...
            if (existingTable && existingTable.id !== id && !existingTable.deletedAt) {
                throw new ConflictException(
    ...
    async getStats(): Promise<any> {
        const [
            ...
            this.prisma.table.count({ where: { deletedAt: null, status: 'DISPONIBLE' } }),
            this.prisma.table.count({ where: { deletedAt: null, status: 'OCUPADA' } }),
            this.prisma.table.count({ where: { deletedAt: null, status: 'RESERVADA' } }),
